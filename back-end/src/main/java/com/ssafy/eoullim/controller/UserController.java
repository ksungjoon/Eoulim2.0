...
public class UserController {
  ...
  public Response<Void> checkId(@NotBlank @RequestBody UserIdCheckRequest request) {
    userService.checkId(request.getUserName());
    return Response.success();
  }

  @PostMapping("/pw-check") // User Password 재확인
  public Response<Void> checkPw(
      @NotBlank @RequestBody UserPwCheckRequest request, Authentication authentication) {
    User user = ClassUtils.getSafeCastInstance(authentication.getPrincipal(), User.class);
